...
def convolve(image, K):
    # grab the spatial dimensions of the image and K
    (iH, iW) = image.shape[:2]
    (kH, kW) = K.shape[:2]

    # allocate memory for the output image, taking care to "pad"
    # the border of the input image so the spatial size (i.e., 
...
# construct averange blurring kernels used to smooth an image
...
# using both our custom 'convolve' fuction and OpenCv's 'filter2D'
# function
kernelBank = (
...
for (kernelName, K) in kernelBank:
    # apply the kernel to the grayscale image using both our costom
